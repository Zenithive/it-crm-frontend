...
interface FilterHandlerProps {
  ...
  onFilterApply: (payload: FilterPayload) => void | Promise<void>; // Support both sync and async
  setShowFilter: (show: boolean) => void;
}

const FilterHandler: React.FC<FilterHandlerProps> = ({ filterSections, onFilterApply, setShowFilter }) => {
            ...
            <div key={option.id} className="flex items-center space-x-3 ml-2 mt-6">
              <input
                ...
                onChange={() => {
                  ...
                  handleFilterApply({
                    filter: {
                      [activeSection]: newSelected[0], // Dynamic key based on active section
                    },
  ...
  const handleFilterApply = async (payload: FilterPayload) => {
    await onFilterApply(payload); // Handle both sync and async cases
