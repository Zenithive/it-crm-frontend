...
//       try {
//         setLoading(true);
//         const response = await overallcasestudyDataApi();
//         console.log("API Response:", response?.data?.data?.getCaseStudies);
  
//         if (response?.data?.data?.getCaseStudies) {
//           const transformedData = response.data.data.getCaseStudies.map((item) => ({
...
const ResourceContainer = () => {
  ...
  const resources = caseStudies.map((item) => ({
    title: item.projectName,
    company: item.clientName,
    tags: item.tags ? item.tags.split(", ").map((tag) => tag.trim()) : [],
